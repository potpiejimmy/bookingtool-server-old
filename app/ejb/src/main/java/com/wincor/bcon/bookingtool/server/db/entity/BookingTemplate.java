...
@NamedQueries({
    ...
    @NamedQuery(name = "BookingTemplate.findBySearchString", query = "SELECT bt FROM BookingTemplate bt,Budget b,Project p WHERE bt.budgetId=b.id AND b.projectId=p.id AND p.domainId=:domainId AND bt.searchString LIKE :searchString AND bt.active = :active")})
